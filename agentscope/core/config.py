"""AgentScopeConfig — typed configuration via Pydantic BaseSettings.

Sources (in priority order):
  1. Init kwargs
  2. Environment variables (prefix AGENTSCOPE_)
  3. .agentscope.toml in CWD or ~/.agentscope/config.toml
...
class SanitiseConfig(BaseSettings):
    ...
    model_config = {"env_prefix": "AGENTSCOPE_SANITISE_", "frozen": True}
...
def _load_toml_config() -> dict[str, Any]:
    """Load config from .agentscope.toml (CWD) or ~/.agentscope/config.toml."""
    candidates = [
        Path.cwd() / ".agentscope.toml",
        Path.home() / ".agentscope" / "config.toml",
    ]
